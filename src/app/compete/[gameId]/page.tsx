...
export default function CompeteGamePage() {
  ...
  useEffect(() => {
    ...
    const fetchResults = async (attempt = 0) => {
      try {
        const res = await fetch(`/api/compete/${gameId}/all-results?playerId=${playerId}`);
  ...
  return (
    <main className={`app-shell ${pageStyles.pageShell}`}>
      ...
      {playerSubmittedToast && (
        <div className={pageStyles.playerSubmittedToast} data-testid="player-submitted-toast">
          {playerSubmittedToast}
        </div>
      )}
