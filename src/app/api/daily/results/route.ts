...
async function loadDailyResults(date: string): Promise<RawDailyRow[]> {
  const result = await dbPool.query<RawDailyRow>(
    `WITH human_daily AS (
      SELECT
        player_id,
        avg_accuracy,
...
export async function GET(request: Request) {
  try {
    ...
    const rankedRows = await loadDailyResults(targetDate);
    ...
    const rows: DailyResultRow[] = rankedRows.map((row) => {
      ...
      return {
        ...
        best_round_accuracy: row.best_round_accuracy != null ? Number(row.best_round_accuracy) : null,
      };
    });
    ...
    console.error("[daily/results] Failed to load daily results:", error);
