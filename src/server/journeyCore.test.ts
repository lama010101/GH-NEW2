...
const mockFns = vi.hoisted(() => ({
  ...
  appendEvent: vi.fn(),
  fetchRandomEventsForSession: vi.fn(),
}));
...
vi.mock("@/server/events", () => ({
  fetchRandomEventsForSession: mockFns.fetchRandomEventsForSession,
}));
...
function createMockClient(opts: { stage: MockStageRow; unlocked: boolean }) {
  ...
  const query = vi.fn(async (sql: string, params?: unknown[]) => {
    ...
    if (sql.includes("FROM public.journey_stages") && sql.includes("FOR UPDATE")) {
      return { rows: [opts.stage] };
    }
    if (sql.includes("jp.status = 'completed'")) {
      return { rows: [{ unlocked: opts.unlocked }] };
    }
    if (sql.includes("FROM public.journey_playthroughs")) {
      // No prior drawn-event history — reaching this proves the guest gate
...
describe("startJourneyPlaythrough — guest gate (HJ-T19)", () => {
  it("rejects an anonymous caller starting stage 2 even when stage 2 is already unlocked", async () => {
    const client = createMockClient({
      stage: { id: "stage-2", stage_number: 2, status: "live" },
      unlocked: true,
    });
    ...
    expect(mockFns.fetchRandomEventsForSession).not.toHaveBeenCalled();
  });
  ...
  it("does not gate a permanent-account caller starting stage 2", async () => {
    ...
    // Reaches the draw (mocked to return zero events) instead of the
  ...
  it("does not gate an anonymous caller starting stage 1", async () => {
    const client = createMockClient({
      stage: { id: "stage-1", stage_number: 1, status: "live" },
      unlocked: true,
    });
    mockFns.getTransactionClient.mockResolvedValue(client);
    mockFns.fetchRandomEventsForSession.mockResolvedValue([]);
