          ...
          <>
            ...
            <div className={styles.leaderboardCard}>
              ...
              {(leaderboardTab === 'thisRound' ? leaderboardRows : allRoundsLeaderboardRows).map(row => {
                ...
                const isAi = snapshot.players.find(p => p.playerId === row.playerId)?.isAi ?? false;
                  ...
                  <div key={row.rank} className={`${styles.lbRow} ${row.isMe ? styles.lbRowSelfAccent : ""}`}>
                    ...
                    <span className={styles.lbNameCell}>
                      <span className={styles.lbNameInner}>
                        <PlayerAvatar avatarUrl={avatarUrl} displayName={row.displayName} playerId={row.playerId} size={40} isMe={row.isMe} isAi={isAi} />
                ...
                snapshot.players.map((p) => {
                  ...
                  return (
                    <div key={p.playerId} className={`${styles.lbRow} ${isMe ? styles.lbRowSelfAccent : ""}`}>
                      ...
                      <span className={styles.lbNameCell}>
                        <span className={styles.lbNameInner}>
                          <PlayerAvatar avatarUrl={p.avatarUrl} displayName={p.displayName || p.playerId.slice(0, 8)} playerId={p.playerId} size={40} isMe={isMe} isAi={p.isAi} />
                      ...
                      {effectiveReadyForNext.map(pid => {
                        ...
                        return (
                          <span key={pid} className={styles.readyName}>
                            <PlayerAvatar avatarUrl={p?.avatarUrl ?? null} displayName={name} playerId={pid} size={24} isMe={pid === playerId} isAi={p?.isAi ?? false} />
