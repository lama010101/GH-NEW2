// ============================================================================
// Multiplayer Scenario Tests: 4 Players, 5 Rounds
// ============================================================================
// Tests various gameplay scenarios:
...
async function runScenario(scenario: ScenarioConfig): Promise<{ passed: boolean; errors: string[] }> {
  ...
  if (gameId) {
    try {
      console.log(`\n[CLEANUP] Deleting test session ${gameId}...`);
      // Note: We'd need a delete API endpoint for this, or manual DB cleanup
      console.log(`[CLEANUP] Session ${gameId} left in DB for manual inspection`);
    } catch (error) {
      console.error(`[CLEANUP] Failed to cleanup session:`, error);
    }
  }
