...
- **Admin `/admin/openrouter` Playwright tests must not `waitForLoadState('networkidle')` on redirects**: A non-admin user is redirected from `/admin/openrouter` → `/login` → `/home`, and `/home` polls `/api/notifications` and `/api/invitations/pending` continuously. Use `page.goto('/admin/openrouter', { waitUntil: 'domcontentloaded' })` (or omit `networkidle`) and assert final URL / absence of `OpenRouter AI Player Activity` text.
