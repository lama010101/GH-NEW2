...
- **Node 20 and `@supabase/supabase-js` realtime WebSocket**: When creating a Supabase service-role client from a standalone Node script (e.g. a test-user setup script), pass `const WebSocket = require('ws')` and set `realtime: { transport: WebSocket }` in the client options. Node 20 does not have native WebSocket support, and omitting the transport causes `RealtimeClient` to throw.
