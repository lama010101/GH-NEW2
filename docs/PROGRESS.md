...
| UI-FIX-HINT-LABEL-002 | DONE | src/app/compete/[gameId]/page.tsx | hint labels use full legacy mapping with per-tier index (Distant Event/Years From Distant Event, Remote Landmark/Distance To Remote Landmark etc.), Home button icon only |
| UI-INV-HINTS-001 | DONE | — | Located hint display component, bottom bar Home button, and Next Round button. All three elements found in src/app/compete/[gameId]/page.tsx: Hints panel (lines 963-978, hint rows at 971-976), Home button (lines 995-1008, emoji icon 🏠), Next Round button (lines 1024-1038, inline styles with orange #f97316 background). |
| UI-FIX-HINTS-BAR-001 | DONE | src/app/compete/[gameId]/page.tsx | Fixed hint rows display, Home button icon, and Next Round button wrapping. Changes: (1) Hint rows now show label:value format using hint.type field (no tier badge), (2) Home button uses inline SVG house icon instead of emoji, (3) Next Round button has whiteSpace:nowrap to prevent text wrapping. TypeScript validation passed (exit code 0). |
| UI-INV-HINT-FIELDS-001 | DONE | — | Inspected hint object shape available on client snapshot. EventHint type (src/core/types.ts:40-48) has 7 fields: id, event_id, tier, type, content, metadata, display_order. DB hints table (migration 019:80-88) matches exactly. Fetch query in getGameState.ts:408-418 selects all 7 columns. |
| UI-INV-HINT-ORDER-001 | DONE | — | Inspected display_order values for tier 2 and tier 4 hints in DB. Finding: Each event has exactly ONE hint per (type, tier) combination (no duplicates). Total counts: 2182 hints for each of 4 combos (when/tier2, when/tier4, where/tier2, where/tier4). All hints have display_order=0. Metadata contains only numeric values (years for when, km for where) — no subtype/label/kind field. Premise of two sub-rows per tier is not supported by current schema. |
...
| MP-BUILD-RESULTS-001 | COMPLETE | src/app/globals.css, src/app/compete/[gameId]/page.tsx | Round Results Page Redesign: Layout reordered (title→image→answer→error→description→score→where→when→leaderboard), spacing reduced in globals.css (.app-shell padding 24→12, .card padding 20→14, .stack gap 16→10, .shell-grid gap 20→10), avatars added to all leaderboard rows (round leaderboard, WHERE sub-leaderboard, WHEN sub-leaderboard), timeline improved with endpoint labels (timelineMin/timelineMax at bottom), correct year marker styling updated (fontSize 11/white/600 above, fontSize 9/#aaa below), player guess markers now show avatars above dots (20×20 border-radius 50%), timeline container height increased 96→120. TypeScript validation passed (exit code 0). |
| MP-BUILD-CARDS-002 | Restructure WHERE/WHEN cards with collapsible minicards, restore clues + badge sounds, fix ESLint | DONE | 2026-05-10 |
