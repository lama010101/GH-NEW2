...
| FIX-WELCOME-MODAL-UI-001 | DONE | src/components/WelcomeModal.tsx, src/components/WelcomeModal.module.css | Welcome modal UI fixes: (1) Removed "Welcome to Guess-History!" greeting h2 (styles.greeting element + .greeting CSS rule removed). (2) Promoted "Your historical avatar" text to be the main title — changed from <p> to <h2> and restyled .avatarIntro from small uppercase muted caption to Bebas Neue var(--font-3xl) primary-color title with 24px bottom margin (matching the old .greeting styling). (3) Fixed edit-avatar gear button being clipped: removed `overflow: hidden` from .avatarWrap (which was cutting off the gear button positioned at right:-4px/bottom:-4px outside the circle), added `border-radius: 50%` to .avatarImg so the image stays circular, and added z-index:2 to .gearButton so it renders above the avatar circle. (4) Lightened muted text in dark mode: overrode --gh-modal-text-muted to rgba(255,255,255,0.62) locally on .card (was 0.40 globally — too dark on dark modal background), with a light-theme reset to rgba(15,23,42,0.55) via :global([data-theme="light"]) .card. Affects .avatarIntro(removed), .bioLine, .usernameLabel, .skipLink muted text. tsc: 0 errors. (2026-07-02) |

| PROTO-LOBBY-SETTINGS-IMAGES-V2-001 | DONE | src/app/prototype/lobby-settings-images/page.tsx, src/app/prototype/lobby-settings-images/lobby-settings-images.module.css, public/prototype/lobby-settings-images/{ancient,medieval,earlymodern,modern,contemporary,africa,asia,europe,north_america,south_america,oceania_antarctica}.jpg | Reworked the lobby-settings-images prototype: (1) Removed V1 (stock-only) section entirely — page now shows only the DB-images version (queries Supabase images+events+locations, falls back to local stock images). (2) Replaced 8 of 11 local stock images with new Unsplash photos: ancient=tall columned building (mnDT6VUehH8), medieval=knight archer (qq1DEOBwRIw, Unsplash+ via plus.unsplash.com), modern=snowy train tracks (gVxlHzUqz7M), africa=giraffe on green field (lY0qFestcG0), asia=brown-gold pagoda + cherry blossom (cddaZDt6uRw), europe=Rialto Bridge Venice (hFXZ5cNfkOk), north_america=Statue of Liberty blue sky (cqp4CnFwI08, free alternative to paid f_yedZMp0eE), south_america=Machu Picchu daytime (tfS_WrLUf40), oceania_antarctica=emperor penguins on ice (Onn6jBF-vx4, free alternative to paid mrSB23USzqE). contemporary (rocket) and earlymodern (tall ship) kept from prior batch — the requested contemporary rocket (X3o5blcQGXA) is Unsplash+ (403 on download) so the existing free rocket image was kept. All images resized to max 400px via sips. Deleted old antarctica.jpg and oceania.jpg (replaced by oceania_antarctica.jpg). (3) Moved Oceania & Antarctica to LAST position in REGIONS array (was 5th of 6, now 6th of 6). (4) Reduced image thumbnail height by 25%: .imgThumb 92px→69px, .imgFallback font 36px→28px, .imgBtn gap 6px→4px. (5) Removed the small "S"/"DB" source badge from top-right corner of image buttons — deleted .sourceBadge/.sourceBadgeDb/.sourceBadgeStock CSS rules and removed isDb prop + badge rendering from ImageEraButton and ImageRegionButton. (6) Added .imgSpacer (10px height) to align Era and Region button label positions: Era buttons render label+span+1spacer (span+spacer below label), Region buttons render label+2spacers (no span, 2 spacers fill same vertical space) — both put the label at the same Y position relative to the thumbnail bottom, and the era year range no longer touches the button bottom edge. Removed legend section (no longer needed without badges). tsc: 0 errors. eslint: clean. (2026-07-03) |

| PROTO-LOBBY-SETTINGS-IMAGES-V3-001 | DONE | src/app/prototype/lobby-settings-images/page.tsx, src/app/prototype/lobby-settings-images/lobby-settings-images.module.css | Two fixes to the lobby-settings-images prototype. (1) Image fallback chain: DB url → local stock image → emoji. Previously when a DB image URL failed to load (e.g. RLS blocked, stale URL, CORS), onError jumped straight to emoji — so new stock images for contemporary, north_america, and oceania_antarctica were never visible. Now ImageEraButton/ImageRegionButton accept dbUrl + stockImg separately: try dbUrl first, on error switch to stockImg, on stockImg error show emoji. Added useEffect to reset src when dbUrl changes (after Refresh). (2) Square buttons with full-bleed image: .imgBtn is now 96x96px position:relative overflow:hidden (was 96px wide with separate 92x69 thumbnail + label below). Image is position:absolute inset:0 object-fit:cover filling the entire button. Label and span are overlaid at the bottom via .imgCaption (position:absolute bottom:0) with text-shadow for legibility over the gradient scrim (.imgOverlay darkened to 0.75 at bottom). Removed .imgThumb, .imgSpacer classes. tsc: 0 errors. eslint: clean. (2026-07-03) |

| PROTO-LOBBY-SETTINGS-IMAGES-V4-001 | DONE | public/prototype/lobby-settings-images/{contemporary,modern,north_america,oceania_antarctica}.jpg | Replaced 4 stock images in the lobby-settings-images prototype with new Unsplash photos (all free-license, downloaded successfully): contemporary=Buzz Aldrin on the moon (e5eDHbmHprg), modern=steam train through lush green forest (yuWewIkdbKw, fallback for the prior snowy-train image), north_america=Statue of Liberty NYC (ZsqtZUAe3u4), oceania_antarctica=group on boat in front of icebergs (FycMSc7sulo). All resized to max 400px via sips. No code changes — images are referenced by existing stockImg paths in page.tsx. (2026-07-03) |

| FEAT-MERGE-OCEANIA-ANTARCTICA-001 | DONE | src/components/compete/LobbySection.tsx, src/components/practice/PracticeSettingsModal.tsx, src/components/compete/LobbySection.module.css, src/i18n/{en,ar,zh,ja,ru,nl,pt,it,de,es,fr}.json | Merged Antarctica and Oceania into a single region "Oceania & Antarctica" across all modes (compete lobby + practice settings). RegionId type changed from 7 ids to 6: removed 'antarctica' and 'oceania', added 'oceania_antarctica'. REGIONS array field changed from continent: string to continents: string[] — the merged region maps to ['Oceania','Antarctica']. All r.continent references updated: visibleRegions filter uses r.continents.some(c => available.includes(c)), continent→regionId mapping uses r.continents.some(c => incoming.includes(c)), toggleRegion/handleStart send continents via flatMap(r => r.continents), toggleAllRegions sends last.continents. CSS: removed [data-era="antarctica"] and [data-era="oceania"] rules, added [data-era="oceania_antarctica"] with --gh-success background. i18n: replaced region_antarctica + region_oceania keys with single region_oceania_antarctica key in all 11 language files with localized combined labels. Backend (events.ts VALID_CONTINENTS) unchanged — still 7 continents in DB; the merge is a UI-layer concern only, the UI expands the merged button to both continent names when sending to the server. tsc: 0 errors. eslint: clean. (2026-07-02) |
...
| FIX-LOBBY-JOIN-SECRET-NEWLINE-001 | DONE | partykit.json | Removed trailing `\n` escape sequence from PARTYKIT_SECRET value in partykit.json. The JSON string `"76d9112...d104\n"` parsed to a 65-char value (64 hex + newline), while Next.js process.env.PARTYKIT_SECRET from .env.local is 64 chars (no newline). verifyPartyKitSecret() in src/server/partykitAuth.ts checks length first (`headerValue.length !== expected.length` → return false), so every DO→API call (loadFromDB on cold start, JOIN_ROOM, TOGGLE_READY, etc.) returned 401. Symptom: clicking "Create game" connected the WebSocket (onBeforeConnect verifies the Supabase user token, which passed) but the DO could never load the snapshot from /api/compete/:gameId (401) and JOIN_ROOM failed (401), so the client never received a valid STATE_UPDATE and stayed on the loading screen — "cannot join lobby". The 1006 WS disconnects in the browser console are a separate dev-mode HMR artifact (Fast Refresh tearing down sockets on rebuild) and unrelated to the join failure. After fix both secrets are 64 hex chars and match. tsc: n/a (config-only). (2026-07-02) |

| FIX-EDGE-CRYPTO-PARTYKITAUTH-001 | DONE | src/server/partykitAuth.ts | Replaced Node.js `crypto.timingSafeEqual` with a runtime-agnostic constant-time comparison (TextEncoder + XOR accumulation) in verifyPartyKitSecret(). The function is imported by src/middleware.ts:105, which runs in the Next.js edge runtime; the edge runtime does not support the Node.js `crypto` module, so every DO→API request (loadFromDB cold start, JOIN_ROOM, /leave) threw a 500 in middleware before reaching the route handler — lobby never loaded. Symptom in dev logs: `The edge runtime does not support Node.js 'crypto' module` at partykitAuth.ts:1, followed by `[PartyKit] Cold start loadFromDB failed: snapshot API error 500` and `[JOIN_ROOM] API error 500`. New implementation: length check early (length is not secret), then encode both strings via TextEncoder (available in edge + node), XOR all bytes into an accumulator with no early exit, return diff===0. Constant-time property preserved on equal-length inputs. Function signature unchanged — all 29 consumers (14 compete routes, sessionCore.ts, middleware.ts) compile unchanged. Grep proof: `from "crypto"` in src/ → 4 matches, all in node-runtime-only files (sessionCore.ts, db.ts, 2 test files); none imported by edge middleware. tsc: 0 errors. (2026-07-02) |
...
| RELAX-G2-LEADERBOARD-WAITING-001 | DONE | src/components/compete/RoundActiveSection.tsx | Compete Relax spec: leaderboards must show "waiting" for each player who has not submitted yet. In the opponent avatars block (snapshot.players filtered to non-self), added a persistent "waiting" label (reusing existing i18n key game.waiting_for, present in all 10 locales) that renders when !p.hasSubmitted && !submittedToasts[p.playerId]. Uses the existing .submittedToast CSS class with inline style overrides (animation:none, opacity:1, tertiary text color) to avoid the 2s fade-out animation that class normally applies — this keeps the change to ONE file (no .module.css edit). The existing "Guessed" toast (t('guessed')) for submitters is unchanged. Single source of truth preserved: p.hasSubmitted from snapshot.players is the sole read. tsc --noEmit: exit 0. (2026-07-03) |

| RELAX-G3A-ABSENT-SCHEMA-001 | DONE | supabase/migrations/20260703000000_add_absent_to_round_commits.sql | Compete Relax (Option B) §6 schema gap: round_commits could not distinguish "submitted a real guess scoring 0" from "never submitted / absent." Added `absent BOOLEAN NOT NULL DEFAULT FALSE` to public.round_commits via ALTER TABLE ADD COLUMN IF NOT EXISTS. Respects the append-only composite-PK design (no new table, no second writer). round_results stays derived. Existing rows backfill to absent=FALSE (real guesses). Absent rows (absent=TRUE, score=0) will be inserted by the round-completion path in sessionCore.ts (sub-slice 3b) for active players who did not submit before round-timer or session-deadline expiry. Single source of truth: one absent column on one table. grep "absent" across migrations: only this file defines the column (024 uses it in a comment). (2026-07-03) |

| RELAX-G3B-ABSENT-MARKING-001 | DONE | src/server/sessionCore.ts | Compete Relax (Option B) §6: mark non-submitters as absent=TRUE. Modified insertMissingCommits (the timer-expiry path called by completeRound) to set absent=TRUE on the zero-score rows it inserts for active players who did not submit before round-timer or session-deadline expiry. Previously these rows got absent=FALSE (the column default from migration 20260703000000), making them indistinguishable from real guesses that scored 0. The submitGuess INSERT path (line 1698) is unchanged — real guesses still get absent=FALSE (column default). Single source of truth: one absent column on round_commits, written by exactly one function for the absent case (insertMissingCommits) and defaulted by the other (submitGuess). tsc --noEmit: exit 0. grep "INSERT INTO round_commits": 2 sites (submitGuess=real guess/absent=FALSE, insertMissingCommits=absent/absent=TRUE), no duplicate logic. (2026-07-03) |

| VERIFY-MIGRATIONS-LAST-10D-001 | DONE | (DB only) supabase_migrations.schema_migrations on staging (gzvixlvkwjsrtmtybtkf) | Verified all migrations created in the past 10 days (2026-06-23 -> 2026-07-03) were applied successfully to the remote staging DB. Cross-checked the 7 local timestamped migration files in supabase/migrations/ against supabase_migrations.schema_migrations rows AND the actual schema objects (information_schema.columns, pg_constraint, pg_policy, to_regclass). All schema changes are live and correct: player_event_ratings table, profiles.locale, waitlist table, sessions.session_deadline_days + sessions_session_deadline_days_range CHECK, sessions.selected_regions, and the 5 participant-scoped RLS SELECT policies (sessions, session_players, round_commits, round_results, round_events). Found and fixed migration-tracking drift: 3 migrations (20260628000000 add_session_deadline_days, 20260630000000 add_selected_regions_to_sessions, 20260702000000 restrict_rls_to_participants) had been applied out-of-band and were NOT registered in schema_migrations; 1 migration (create_waitlist) was registered with version 20260626111106 instead of the local file's 20260626000000. Reconciliation (single transaction, committed): UPDATE waitlist version 20260626111106 -> 20260626000000; INSERT the 3 missing rows with created_by=laurent.martenot@gmail.com. Post-verify: 0 stale rows, all 8 window migrations registered with versions matching local files. NOTE: a new untracked local file 20260703000000_add_absent_to_round_commits.sql (created today 07:45, not yet committed) is NOT applied and NOT registered -- out of scope for this verification task; flagged for separate apply decision. (2026-07-03) |
