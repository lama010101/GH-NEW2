...
| FEAT-LOBBY-REGION-SELECTOR-001 | DONE | supabase/migrations/20260630000000_add_selected_regions_to_sessions.sql (new), src/core/types.ts, src/server/getGameState.ts, src/server/sessionCore.ts, src/app/api/compete/[gameId]/region-selection/route.ts (new), src/core/competeWebSocket.ts, partykit/server.ts, src/components/compete/LobbySection.tsx, src/components/compete/LobbySection.module.css, src/app/compete/[gameId]/page.tsx, src/i18n/en.json, src/i18n/de.json, src/i18n/es.json, src/i18n/fr.json, src/i18n/it.json, src/i18n/ja.json, src/i18n/nl.json, src/i18n/pt.json, src/i18n/ru.json, src/i18n/zh.json | Added region (continent) selector to lobby settings, mirroring era-presets UIX. Host can toggle 11 continents (Africa, Antarctica, Arctic, Asia, Australia, Central America, Europe, Mars, North America, Oceania, South America) with fixed icons; guests see host's selection in real-time via WS broadcast (disabled buttons, same as era presets for guests). Data flow: LobbySection → competeWebSocket.setRegionSelection → PartyKit SET_REGION_SELECTION → /api/compete/[gameId]/region-selection → setCompeteRegionSelection (UPDATE sessions.selected_regions + refetch events with region filter + update SESSION_CREATED eventIds) → broadcast. DB: new selected_regions TEXT[] column (default '{}' = all regions) — migration applied to remote DB. setCompeteEraSelection updated to pass current selected_regions to event refetch so era changes don't drop region filter. Region chips filtered by fetchAvailableRegions() on mount (hides continents with no events). Default: all regions selected. i18n: region_presets + 11 region_* keys in all 10 locales. tsc: 0 errors in modified files, build: success. DB verification: column exists, all DB continents covered by REGIONS constant, existing rows have default '{}'. (2026-06-30) |
