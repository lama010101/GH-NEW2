...
| FIX-RANKCARD-TITLE-GLOBAL-FONT-001 | DONE | src/components/RankCard.module.css | Replaced hardcoded `font-size: 17px` on `.rankTitle` (line 96) with `font-size: var(--font-xl)` (= 20px) so the rank card title follows the global CSS typography scale and matches the home page title size (home page `.cardTitleLeft` and `.tagline` both use `var(--font-xl)`, defined in src/app/globals.css:194). Single file, single rule, single behavior change. Grep proof: `rankTitle` in RankCard.module.css → 1 match (line 95), no duplicate title-size logic in RankCard.tsx. RankProgressBar.module.css has a separate `.rankTitle` (different component, icon-based progress bar, not the home-page rank card) — untouched. (2026-07-07) |
