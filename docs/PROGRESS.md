...
|---------|--------|---------------|-------|
| MP-RUN-SIMULATION-PROD-002 | BLOCKED | docs/PROGRESS.md | Ran MP-AUTO-TEST-001 (scripts/test/playwright/specs/multiplayer-simulation.spec.ts) against production base URL https://www.guess-history.com on Node v20.20.2 (.nvmrc pin) with production Supabase project gzvixlvkwjsrtmtybtkf.supabase.co. RESULT: FAIL/PARTIAL — 0 of 2 tests executed; 0/3 games, 0/15 rounds, 0/6 players, 0/14 edge cases. Suite errored at module load in fixtures/auth.ts:16 (`const supabase = createClient(...)`): "Node.js 20 detected without native WebSocket support" — installed @supabase/realtime-js requires Node >=22 native WebSocket (or `ws` transport). Playwright JSON stats: expected:0, skipped:0, unexpected:0, flaky:0; errors:2 (globalSetup + globalTeardown both import auth.ts). globalSetup never reached, so NO production test users were created/deleted. Contradiction: task mandates Node 20 but committed harness deps require Node 22; not resolved (no code changes permitted). Also note .env.local NEXT_PUBLIC_PARTY_KIT_HOST=127.0.0.1:1999 (localhost, not a production PartyKit host). (2026-06-19) |
