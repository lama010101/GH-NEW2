...
| MP-FIX-RESULT-002 | DONE | partykit/server.ts | Fixed READY_NEXT double-read body crash; added diagnostic logs for status check. Changes: (1) Moved response.json() inside else block to prevent crash when response.ok is false (body already consumed by text()), (2) Added diagnostic log before status check showing snapshot.status and isRuntimeState, (3) Added diagnostic log before applySnapshotAndBroadcast in SUBMIT_GUESS showing response status and isRuntimeState. Validation: npx tsc --noEmit exits 0. |
