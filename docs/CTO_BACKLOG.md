...
| # | Item | Status | Notes |
|---|------|--------|-------|
| B1 | Home > Compete card invite link sometimes unresponsive until browser refresh | INVESTIGATING (Phase B complete, awaiting CTO review) | Phase B evidence gathered under MP-INV-INVITE-LINK-UNRESPONSIVE-B-001: 4 scenarios tested (3 FAIL, 1 code-read). Key findings: no fetch timeout in handleCreate/handleJoin, no try/catch in handleAccept, identity.ts can flicker to unauthenticated during token refresh causing AuthModal (z-index 1000) to cover Compete card. |
