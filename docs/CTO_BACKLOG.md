...
| # | Item | Status | Notes |
|---|------|--------|-------|
| B1 | Home > Compete card invite link sometimes unresponsive until browser refresh | OPEN — needs A0 | Likely stale WebSocket/event-listener bug. Same suspected family as sign-in/out intermittency (Section A) — worth checking for a shared root cause, but do not assume they're the same bug without evidence. |
